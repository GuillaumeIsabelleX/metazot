...
{var zpItemsFlag=!0;var zpItemNum=1;var zpLastTerm="";var zpSearchBarParams="";var zpSearchBarSource=zpShortcodeAJAX.ajaxurl+"?action=zpRetrieveViaShortcode&zpShortcode_nonce="+zpShortcodeAJAX.zpShortcode_nonce;var zpShowImages=!1;if(jQuery("#METAZOT_AC_IMAGES").length>0)zpShowImages=!0;function mz_set_lib_searchbar_params(filter,start,last)
...
last=!1;zpSearchBarParams="";zpSearchBarParams+="&api_user_id="+jQuery("#METAZOT_USER").val();zpSearchBarParams+="&item_type=items";zpSearchBarParams+="&downloadable="+jQuery("#METAZOT_AC_DOWNLOAD").val();zpSearchBarParams+="&style="+jQuery("#ZP_STYLE").text();zpSearchBarParams+="&sort_by="+jQuery("#ZP_SORTBY").text();zpSearchBarParams+="&order="+jQuery("#ZP_ORDER").text();zpSearchBarParams+="&citeable="+jQuery("#METAZOT_AC_CITE").val();if(jQuery("#METAZOT_AC_MAXRESULTS").val().length>0)
zpSearchBarParams+="&maxresults="+jQuery("#METAZOT_AC_MAXRESULTS").val();if(zpShowImages)zpSearchBarParams+="&showimage=true";if(start)zpSearchBarParams+="&request_start="+start;if(last)zpSearchBarParams+="&request_last="+last;zpSearchBarParams+="&update=true";if(filter)
...
mz_set_lib_searchbar_params(!1,!1,!1);jQuery("input[name='zpSearchFilters']").click(function()
{if(jQuery("input[name=zpSearchFilters]").length>0)
mz_set_lib_searchbar_params(jQuery(this).val(),!1,!1);jQuery("input#zp-Metazot-SearchBox-Input").autocomplete("option","source",zpSearchBarSource+zpSearchBarParams);if(jQuery("input#zp-Metazot-SearchBox-Input").val().length>0&&jQuery("input#zp-Metazot-SearchBox-Input").val()!=zpShortcodeAJAX.txt_typetosearch)
jQuery("input#zp-Metazot-SearchBox-Input").autocomplete("search")});jQuery("input#zp-Metazot-SearchBox-Input").bind("keydown",function(event)
...
{if(jQuery.trim(jQuery(this).val())==""){jQuery(this).val(zpShortcodeAJAX.txt_typetosearch);jQuery(this).addClass("help")}}).autocomplete({source:zpSearchBarSource+zpSearchBarParams,minLength:jQuery("#METAZOT_AC_MINLENGTH").val(),focus:function(){return!1},search:function(event,ui)
...
{if(zpItemsFlag==!0)window.zpACPagination(zpItemsFlag,!1);else window.zpACPagination(zpItemsFlag,!0);zpItemsFlag=!1;mz_set_lib_searchbar_params(!1,ui.content[1].request_next,ui.content[1].request_last);jQuery("input#zp-Metazot-SearchBox-Input").autocomplete("option","source",zpSearchBarSource+zpSearchBarParams);jQuery("input#zp-Metazot-SearchBox-Input").autocomplete("search")}
