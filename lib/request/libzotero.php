...
class Items
{
    ...
    public function writeItem($item){
      $newarritem = array($item);
      //echo '<pre>'; print_r($newarritem); echo '</pre>';
        return $this->writeItems($newarritem);
    }
        ...
        foreach($chunks as $chunk){
          echo '<pre>'; print_r($chunk); echo '</pre>';
            ...
            foreach($chunk as $item){
                $writeArray[] = array_merge($items->pristine, $items->apiObj['data']);
            }
            $requestData = json_encode($writeArray);
...
class Net
{
    ...
    public function request($url, $method="GET", $body=NULL, $headers=array(), $basicauth=array()) {
        ...
        libZoteroDebug(print_r($httpHeaders));
        libZoteroDebug(print_r($body));
